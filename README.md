...
- fork the repo
- makes changes to the files
- create a PR
- To demo/test code
  - Run `yarn build`
  - To host the lib files on the demo branch, run `git add --force lib` to commit the `/lib` folder (which is ignored) to a seperate release canidate or demo branch.
- **HIGHLY RECOMMENDED** to run `yarn cache clean` to avoid having issues where you installed the repo yet the dependency never installs due to the cache never clearing.
- Alternatively move the repo into your codebase and install like so `yarn add --dev ./eslint-plugin-react-native-a11y`
